...
client.on("ready", async () => {
  try {
    ...
    async function getContactInfo(idSerialized) {
      if (contactCache.has(idSerialized)) return contactCache.get(idSerialized);
    ...
    if (looksLikeGroupId(GROUP_INPUT)) {
      ...
      if (EXACT_MATCH) {
        targetGroup =
          groups.find((g) => normalizeName(g.name) === needle) || null;
      } else {
        targetGroup =
          groups.find((g) => normalizeName(g.name).includes(needle)) || null;
      }
    }
    ...
    for (const g of groups) {
      ...
      withInfos.sort((a, b) => (a.number || "").localeCompare(b.number || ""));
